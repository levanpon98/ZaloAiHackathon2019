...
def DepthwiseConvBlock(kernel_size, strides, name, freeze_bn=False):
    ...
    return reduce(lambda f, g: lambda *args, **kwargs: g(f(*args, **kwargs)), (f1, f2, f4, f3))
...
def build_regress_head(width, depth, num_anchors=9, freeze_bn=False):
    options = {
        'kernel_size': 3,
        'strides': 1,
        'padding': 'same',
...
def build_class_head(width, depth, num_classes=20, num_anchors=9, freeze_bn=False):
    options = {
        'kernel_size': 3,
        'strides': 1,
        'padding': 'same',
...
def efficientdet(phi, num_classes=20, weighted_bifpn=False, freeze_bn=False, score_threshold=0.01):
    ...
    regress_head = build_regress_head(w_head, d_head, freeze_bn=freeze_bn)
    class_head = build_class_head(w_head, d_head, num_classes=num_classes, freeze_bn=freeze_bn)
