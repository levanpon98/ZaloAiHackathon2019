...
def create_callbacks(training_model, prediction_model, validation_generator, args):
    ...
    if args.snapshots:
        ...
        checkpoint = keras.callbacks.ModelCheckpoint(
            os.path.join(
                args.snapshot_path,
                '{dataset_type}_{{epoch:02d}}_{{loss:.4f}}.h5'.format(dataset_type=args.dataset_type)
            ),
            verbose=1,
            # save_best_only=True,
            # monitor="mAP",
...
def parse_args(args):
    ...
    coco_parser.add_argument('json_name', help='Name of json file to parse.')
